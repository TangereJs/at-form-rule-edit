(function(reNamespace) {
  ...
  ActionsBuilder.prototype = {
    init: function() {
      ...
      this.element.actionsHtml = actions;
    },
    ...
    buildActions: function(data) {
      ...
      for(var i=0; i < data.length; i++) {
        ...
        while(field = fields.shift()) {
          ...
          var findResult = actionDiv.querySelectorAll(selector);
    ...
    buildField: function(field) {	  
      ...
      if(field.fieldType == "select") {
        ...
        for(var i=0; i < field.options.length; i++) {
          ...
          var optionDataAttr = document.createAttribute("optionData");
          optionDataAttr.value = JSON.stringify(optionData);
          option.attributes.setNamedItem(optionDataAttr);
          select.appendChild(option);
        }
    ...
    collectData: function(fields) {
      var _this = this;
      fields = fields || this.element.actionsHtml.querySelectorAll(".action");
      var out = [];
      if (fields.length > 0){
        for(var p=0; p < fields.length; p++){
          ...
          var subfields = field.querySelectorAll(".subfields > .field");
          var action = {name: input.name, value: input.value};
  ...
  function selectInputs(field){
    ...
    var result2 = field.querySelector(".jstEditor");
  ...
  function selectSelectTextAreaOrInput(field){
      var results = [];
      var tmpResults = field.getElementsByTagName("select");
      ...
      var tmpResults = field.getElementsByTagName("input");
      if (tmpResults !== undefined && tmpResults.length > 0){
        for( var i=0; i< tmpResults.length; i++){
          results.push(tmpResults[i]);
        }
      ...
      var tmpResults = field.getElementsByTagName("textarea");
      if (tmpResults !== undefined && tmpResults.length > 0){
        for( var i=0; i< tmpResults.length; i++){
          results.push(tmpResults[i]);
        }
...
})(window.RuleEngineHelpers = window.RuleEngineHelpers || {});
