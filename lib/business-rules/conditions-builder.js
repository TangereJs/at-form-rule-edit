      ...
      if (klass) {
        ...
        if (rules.length > 0) {
          for (var k = 0; k < rules.length; k += 1) {
            out.conditions.push(_this.collectDataFromNode(rules[k]));
          }
        }
...
//      removeLink.value = "Remove This Sub-Condition";
    ...
    buildRule: function (ruleData) {
      ruleData.compareTo = ruleData.compareTo !== undefined ? ruleData.compareTo : 'static';
      ...
      fieldSelect.addEventListener('change', function (e) {
        ...
        var compareToValue = fieldRuleData.compareTo !== undefined ? fieldRuleData.compareTo : 'static';
        ...
        if (operators !== undefined) {
          for (var i = 0; i < operators.length; i++) {
            ...
            option.setAttribute('data-compare-to', compareToValue);
      ...
      // classValueElems = Polymer.dom(ruleDiv).querySelectorAll(".compareTo");
      // for (j = 0; j < classValueElems.length; j++) {
      //   var cvElem = classValueElems[j];
      //   cvElem.value = ruleData.compareTo;
      // }
...
//    removeLink.text = "Remove";
  ...
  function onOperatorSelectChange(e) {
    ...
    if (option !== undefined) {
      ...
      var compareToValue = option.getAttribute('data-compare-to');
      switch (fieldType) {
      case "none":
      ...
      case "select":
        ...
        if (compareToValue === 'field') {
          Polymer.dom(nElem).classList.add('compareTo');
        } else {
          Polymer.dom(nElem).classList.add("value");
        }

        var checkedOption = Polymer.dom(fieldSelect).querySelector("option:checked");
        var options = checkedOption.getAttribute("data-options");
        options = JSON.parse(options);
        for (var i = 0; i < options.length; i++) {
          var opt = options[i];
          var htmlOption = createSelectOption(opt.name, opt.label || opt.name, compareToValue === opt.name);
